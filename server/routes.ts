import type { Express } from "express";
import { createServer, type Server } from "http";
import { storage } from "./storage";
import { getAppointments, getAppointmentById, getFormItems } from "./formsiteApi";
import { appointmentFiltersSchema } from "@shared/schema";
import { fromZodError } from "zod-validation-error";

export async function registerRoutes(app: Express): Promise<Server> {
  // Get filtered appointments
  app.get("/api/appointments", async (req, res, next) => {
    try {
      // Parse and validate filters
      const result = appointmentFiltersSchema.safeParse(req.query);
      if (!result.success) {
        const validationError = fromZodError(result.error);
        return res.status(400).json({ message: validationError.message });
      }

      const filters = result.data;
      
      try {
        const appointments = await getAppointments(filters);
        res.json(appointments);
      } catch (apiError) {
        console.error("Error fetching appointments:", apiError);
        // Return the actual error for debugging the API connection
        return res.status(500).json({ 
          message: apiError instanceof Error ? apiError.message : "Unknown error occurred",
          error: "formsite_api_error"
        });
      }
    } catch (error) {
      next(error);
    }
  });

  // Get appointment by ID
  app.get("/api/appointments/:id", async (req, res, next) => {
    try {
      const { id } = req.params;
      
      try {
        // Get all appointments (this will use the cache we built in the API)
        const allAppointments = await getAppointments();
        
        // Find the appointment directly from the list
        const appointment = allAppointments.find(app => app.id === id);
        
        if (!appointment) {
          return res.status(404).json({ message: "Appointment not found" });
        }
        
        res.json(appointment);
      } catch (apiError) {
        console.error(`Error fetching appointment ${id}:`, apiError);
        // Return a 404 instead of 500 for a better UX when appointment not found
        return res.status(404).json({ 
          message: "Appointment not found",
          error: "not_found"
        });
      }
    } catch (error) {
      next(error);
    }
  });

  // Get filter options (set by, provider, marketing channel)
  app.get("/api/filters", async (req, res, next) => {
    try {
      // We'll extract these from the Formsite API in a real implementation
      // For now, return static data based on our content MD file
      const filters = {
        setBy: ["Seth", "Sera"],
        provider: ["Sera", "Courtesan Couple", "Chloe", "Alexa", "Frenchie"],
        marketingChannel: ["Private Delights", "Eros", "Tryst", "P411", "Slixa", "Instagram", "X", "Referral"]
      };
      
      res.json(filters);
    } catch (error) {
      next(error);
    }
  });
  
  // Appointment Actions
  
  // Reschedule appointment
  app.post("/api/appointments/:id/reschedule", async (req, res, next) => {
    try {
      const { id } = req.params;
      const { updatedStartDate, updatedStartTime, updatedEndDate, updatedEndTime } = req.body;
      
      // In a real implementation, this would update the appointment in Formsite
      // For now, we'll just return success with mock data
      
      // Validation
      if (!updatedStartDate || !updatedStartTime || !updatedEndDate || !updatedEndTime) {
        return res.status(400).json({ message: "Missing required fields for rescheduling" });
      }
      
      // Mock success response
      setTimeout(() => {
        res.json({ 
          success: true, 
          message: "Appointment rescheduled successfully",
          data: {
            id,
            updatedStartDate,
            updatedStartTime,
            updatedEndDate,
            updatedEndTime,
          }
        });
      }, 500); // Simulate server delay
    } catch (error) {
      next(error);
    }
  });
  
  // Complete appointment
  app.post("/api/appointments/:id/complete", async (req, res, next) => {
    try {
      const { id } = req.params;
      
      // In a real implementation, this would update the appointment in Formsite
      // For now, we'll just return success with mock data
      
      // Mock success response
      setTimeout(() => {
        res.json({ 
          success: true, 
          message: "Appointment marked as completed",
          data: {
            id,
            dispositionStatus: "Completed",
          }
        });
      }, 500); // Simulate server delay
    } catch (error) {
      next(error);
    }
  });
  
  // Cancel appointment
  app.post("/api/appointments/:id/cancel", async (req, res, next) => {
    try {
      const { id } = req.params;
      const { whoCanceled, cancellationDetails } = req.body;
      
      // In a real implementation, this would update the appointment in Formsite
      // For now, we'll just return success with mock data
      
      // Validation
      if (!whoCanceled) {
        return res.status(400).json({ message: "Must specify who canceled the appointment" });
      }
      
      // Mock success response
      setTimeout(() => {
        res.json({ 
          success: true, 
          message: "Appointment canceled successfully",
          data: {
            id,
            dispositionStatus: "Canceled",
            whoCanceled,
            cancellationDetails,
          }
        });
      }, 500); // Simulate server delay
    } catch (error) {
      next(error);
    }
  });

  // Get Formsite form structure (for debugging)
  app.get("/api/form-items", async (req, res, next) => {
    try {
      try {
        const formItems = await getFormItems();
        res.json(formItems);
      } catch (apiError) {
        console.error("Error fetching form items:", apiError);
        return res.status(500).json({ 
          message: apiError instanceof Error ? apiError.message : "Unknown error occurred",
          error: "formsite_api_error"
        });
      }
    } catch (error) {
      next(error);
    }
  });

  const httpServer = createServer(app);
  return httpServer;
}
